...
def path_is_within(rel_path: str, allowed_path: str) -> bool:
...
def git_changed_files(repo_root: Path) -> set[str]:
    ...
    for raw_line in result.stdout.splitlines():
        ...
        if " -> " in line:
            ...
            changed.add(normalize_repo_path(old_path))
            changed.add(normalize_repo_path(new_path))
        else:
            changed.add(normalize_repo_path(line))
    return changed
...
def validate_report(
    ...
    normalized_allowed = [normalize_repo_path(path) for path in packet["allowed_paths"]]
    ...
    if history:
        ...
        previous_touched = {normalize_repo_path(path) for path in previous["files_touched"]}
    ...
    if check_git:
        actual_changed = {
            ...
            if any(path_is_within(path, allowed) for allowed in normalized_allowed)
        }
